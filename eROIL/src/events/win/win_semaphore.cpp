...
namespace eroil::evt {
    ...
    static SemErr do_wait(sem_handle handle, DWORD timeout_ms) {
        if (handle == nullptr) return SemErr::NotInitialized;

        DWORD rc = ::WaitForSingleObject(as_native(handle), timeout_ms);
        switch (rc) {
            case WAIT_OBJECT_0: return SemErr::None;
            case WAIT_TIMEOUT: return (timeout_ms == 0) ? SemErr::WouldBlock : SemErr::Timeout;
            case WAIT_FAILED: return SemErr::SysError;
    ...
    SemErr Semaphore::post() {
        ...
        if (!::ReleaseSemaphore(m_sem, 1, nullptr)) {
            ...
            if (err == ERROR_TOO_MANY_POSTS) return SemErr::MaxCountReached;
