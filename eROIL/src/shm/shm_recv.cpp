...
namespace eroil::shm {
    ...
    void ShmRecv::reinit() {
        ...
        if (hdr->state.load(std::memory_order_acquire) != SHM_READY) {
            ERR_PRINT("found existing shm recv block that was in INIT state, weird");
        }
    ...
    ShmRecvErr ShmRecv::read_data(ShmRecvPayload& out) {
        ...
        out.buf = std::make_unique<std::byte[]>(static_cast<size_t>(rec_hdr->payload_size));
        ...
        // LOG("END: cosumed=", rec_hdr->total_size);
