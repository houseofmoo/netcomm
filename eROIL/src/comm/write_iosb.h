...
namespace eroil::comm {
    inline void write_recv_iosb(hndl::RecvHandle* handle,
        ...
        if (handle->data.iosb == nullptr || handle->data.num_iosb <= 0) {
            return;
        }
        ...
        iosb::ReceiveIosb* iosb = handle->data.iosb + handle->data.iosb_index;
        iosb->Status = 0;
        iosb->Reserve1 = 0;
        iosb->Header_Valid = 1;
        ...
        handle->data.iosb_index = (handle->data.iosb_index + 1) % handle->data.num_iosb;
    }

    inline void write_send_iosb(hndl::SendHandle* handle,
                                const NodeId source_id,
                                const Label label,
                                const size_t size,
            ...
            iosb::SendIosb* iosb = handle->data.iosb + handle->data.iosb_index;
            iosb->Status = status; 
            iosb->Reserve1 = 0;
            iosb->Header_Valid = 1;
