...
//     void Broadcast::set_new_publishers(const BroadcastMessage& msg) {
...
//     void Broadcast::remove_subscription(const BroadcastMessage& msg) {
//         // all the labels we send
//         auto send_labels = m_router.get_send_labels();
//         for (const auto& [label, _] : send_labels) {
//             // if theyre not a recver of this label, ignore it
//             if (!m_router.is_send_subscriber(label, msg.id)) continue;

//             // they're a subscriber, do they still subscriber to this label?
//             auto it = std::find(
//                 msg.recv_labels.begin(),
//                 msg.recv_labels.end(),
//                 label
//             );
//             if (it != msg.recv_labels.end()) continue;

//             // they no longer subscriber to this label, remove them
//             const auto addr = m_address_book.get(msg.id);
//             switch (addr.kind) {
//                 case RouteKind::Shm: {
//                     // remove them from the shm memory notifications list
//                     m_router.remove_local_send_subscriber(label, msg.id);
//                     break;
//                 }
//                 case RouteKind::Socket: {
//                     // remove them from the socket notifications list
//                     m_router.remove_remote_send_subscriber(label, msg.id);
//                     break;
//                 }
//                 default: {
//                     ERR_PRINT("tried to add send subscriber but did not know routekind");
...
//     void Broadcast::remove_publisher(const BroadcastMessage& msg) {
//         // all the labels we recv
//         auto recv_labels = m_router.get_recv_labels();
//         for (const auto& [label, _] : recv_labels) {
//             // are they a publisher of this label, if not continue
//             if (!m_router.is_recv_publisher(label, msg.id, m_id)) continue;

//             // they are a publisher, do they still publish this label?
//             auto it = std::find(
//                 msg.send_labels.begin(),
//                 msg.send_labels.end(),
//                 label
//             );
//             if (it != msg.send_labels.end()) continue;
...
//                 case RouteKind::Shm: {
//                     // remove them from the shm memory notifications list
//                     m_router.remove_local_recv_publisher(label, msg.id);
//                     break;
//                 }
//                 case RouteKind::Socket: {
//                     // remove them from the socket notifications list
//                     m_router.remove_remote_recv_publisher(label, msg.id);
//                     break;
//                 }
//                 default: {
//                     ERR_PRINT("tried to add send subscriber but did not know routekind");
